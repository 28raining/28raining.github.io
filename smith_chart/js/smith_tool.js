...
function drawMakerTable() {
  ...
  var inner =
    "<table class='table table-striped table-sm'><tr><th>Real</th><th>Imaginary</th><th>Name</th><th></th></tr>";
  inner +=
    "<tr><td><input type='text' id='customMarkerRe' style='width:75px'></td><td><input type='text' id='customMarkerIm' style='width:75px'></td><td></td><td><button onclick=addCustomMarker()>add</button></td></tr>";
  var i = 0;
...
schematic.push({
  ...
  freq: 2440,
  er: 1,
  freq_unit: { multiplier: 1e6 },
  span: 0.0,
  span_unit: { multiplier: 1e6 },
});
...
function update_schem_component(freq_here, save_impedance, sch_index) {
  ...
  switch (schematic[sch_index].type) {
    ...
    case "rlc":
      re_here = (schematic[sch_index].abs[0] * scaler[0]) / zo;
      im_here =
        (schematic[sch_index].abs[1] * scaler[1] * 2 * Math.PI * freq_here -
          1 / (schematic[sch_index].abs[2] * scaler[2] * 2 * Math.PI * freq_here)) /
        zo;
      break;
    case "rl":
      re_here = (schematic[sch_index].abs[0] * scaler[0]) / zo;
    ...
    case "customZ":
      ...
      if (indexRes == -1) {
        //frequency is greater than largest value in lut
        ...
        re_here =
          schematic[sch_index].lut[indexRes - 1][1] +
          frac * (schematic[sch_index].lut[indexRes][1] - schematic[sch_index].lut[indexRes - 1][1]);
        im_here =
          schematic[sch_index].lut[indexRes - 1][2] +
          frac * (schematic[sch_index].lut[indexRes][2] - schematic[sch_index].lut[indexRes - 1][2]);
      }
...
function impedanceToReflectionCoefficient(real_old, imag_old, zo) {
  ...
  var reflection_mag = Math.sqrt(
    reflectio_coeff_real * reflectio_coeff_real + reflectio_coeff_imag * reflectio_coeff_imag
  );
...
function update_smith_chart() {
  ...
  for (xx = 0; xx < tolElements[1].length; xx++) {
    ...
    for (i = 2; i < schematic.length; i++) {
      ...
      for (sp = 0; sp <= 2 * span_res; sp++) {
        ...
        if (schematic[i].type == "ss" || schematic[i].type == "so") {
          ...
          var temp_array = arc_smith_points(
            start[0],
            start[1],
            ln_length,
            schematic[i].line_zo,
            schematic[i].type,
            true,
            (2 * Math.PI * frequency_at_sp * Math.sqrt(schematic[0].er)) / 3e8,
            start_at_qtr_wl
          );
          ...
          schematic[i].type[0] == "p" ||
          schematic[i].type == "rlc" ||
          schematic[i].type == "rc" ||
          schematic[i].type == "rl"
        ) {
          ...
          schematic[i].type[0] == "s" ||
          schematic[i].type[0] == "b" ||
          schematic[i].type == "customZ" ||
          schematic[i].type == "rlc_par"
        ) {
          //For series elements plotted on normal curves....
          start_impedance[0] = span_impedance_re[sp];
          start_impedance[1] = span_impedance_im[sp];
          var temp_array = arc_smith_points(
            span_impedance_re[sp],
            span_impedance_im[sp],
            re + span_impedance_re[sp],
            im + span_impedance_im[sp],
            "impedance",
            false
          );
        ...
        if (sp == span_res) {
          ...
          dataPoints.push({
            re: (zo * Number(start_impedance[0])).toPrecision(3),
            im: (zo * Number(start_impedance[1])).toPrecision(3),
          });
          if (show_labels_DP) {
            layout_shapes.push({
              type: "circle",
            ...
            if (idealArc)
              textbox_trace.push({
                x: [Number(start_x_coord) + 0.04],
                y: [Number(start_y_coord) - 0.03],
                text: ["DP" + (i - 1)],
                mode: "text",
              });
          }
    ...
    if (show_labels_DP) {
      ...
      if (idealArc)
        textbox_trace.push({
          x: [Number(end_x_coord) + 0.04],
          y: [Number(end_y_coord) - 0.03],
          text: ["DP" + (i - 1)],
          mode: "text",
        });
    }
  ...
  if (span_impedance_re.length > 1) {
    ...
    textbox_trace.push({
      x: [Number(sp_coord_x[0]) - x_off],
      y: [Number(sp_coord_y[0]) - y_off],
      text: ["F-span"],
      mode: "text",
    });
...
function draw_schematic(i) {
  ...
  innerText +=
    ...
    sch_svg +
    ' 0 500 500"><use xlink:href="svg/elements_w_custom.svg#rainbow3" alt="' +
    sch_label +
    '" /></svg></div></div>';
  ...
  for (cntR = 0; cntR < rows_to_create.length; cntR++) {
    ...
    for (cntC = 0; cntC < rows_to_create[cntR].length; cntC++) {
      ...
      if (boxType == "tol") {
        ...
        if (sch_real && sch_imag)
          innerText += Number(Math.abs(schematic[i].imaginary * zo).toPrecision(precision)) + "j";
        else if (sch_imag) innerText += Number((schematic[i].imaginary * zo).toPrecision(precision)) + "j";
        innerText += "</div>";
      } else if (boxType == "custom") {
        innerText +=
          '<button type="button" class="btn btn-secondary m-auto" data-bs-toggle="modal" data-bs-target="#customZModal" onclick="createCustomZModal(' +
          i +
        ...
        innerText +=
          '<input type="text" class="form-control inputMW" value=' +
...
function define_labels() {
  ...
  if (show_labels_adm) {
    trace_adm = {
      ...
      text: [
        "<b>" + (1000 / 4 / zo).toPrecision(3) + "</b>m",
        "<b>" + (1000 / 2 / zo).toPrecision(3) + "</b>m",
        "<b>" + (1000 / zo).toPrecision(3) + "</b>m",
        "<b>" + ((1000 * 2) / zo).toPrecision(3) + "</b>m",
        "<b>" + ((1000 * 5) / zo).toPrecision(3) + "</b>m",
        "<b>" + ((1000 * 10) / zo).toPrecision(3) + "</b>m",
      ],
      mode: "text",
      textfont: {
        color: color_adm,
...
function download2() {
  ...
  element.setAttribute(
    "href",
    "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(schematic, null, "\t"))
  );
